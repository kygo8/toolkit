...
const en = {
  ...
  tools: {
    ...
    cidr: { title: 'CIDR Calculator', desc: 'Calculate IPv4 network, broadcast, host range, subnet mask, wildcard and address count.' },
    userAgent: { title: 'User-Agent Parser', desc: 'Parse a User-Agent string into browser, OS, device and engine details.' },
    httpHeader: { title: 'HTTP Header Parser', desc: 'Parse raw HTTP request or response headers into a table with common-header notes.' },
    ...
    jwt: { title: 'JWT Decoder', desc: 'Parse JWT header and payload content quickly.' },
    ...
    uuid: { title: 'UUID Generator', desc: 'Generate UUID v4 values in batches.' },
    ...
    qrWifi: { title: 'Wi-Fi QR Code', desc: 'Generate a WIFI: QR code from SSID, encryption, password and hidden network settings.' },
    qrVcard: { title: 'vCard QR Code', desc: 'Build a vCard 3.0 contact and generate a scannable QR code.' },
    ...
    ogPreview: { title: 'Open Graph Preview', desc: 'Generate Open Graph and Twitter Card meta tags with a simple share-card preview.' },
    jsonLd: { title: 'JSON-LD Generator', desc: 'Create guided JSON-LD for WebSite, Article, Organization and BreadcrumbList.' },
...
const zhCN = {
  ...
  tools: {
    ...
    cidr: { title: 'CIDR 计算器', desc: '计算 IPv4 网络地址、广播地址、主机范围、子网掩码、通配符和地址数量。' },
    userAgent: { title: 'User-Agent 解析', desc: '解析 User-Agent，识别浏览器、操作系统、设备和引擎。' },
    httpHeader: { title: 'HTTP Header 解析', desc: '把原始 HTTP 请求或响应头解析成表格，并提供常见 Header 说明。' },
    ...
    jwt: { title: 'JWT 解码', desc: '解析 JWT Token 的 Header、Payload 内容。' },
    ...
    qrWifi: { title: 'WiFi 二维码', desc: '根据 SSID、加密方式、密码和隐藏网络生成 WIFI: 二维码。' },
    qrVcard: { title: '名片二维码', desc: '通过表单生成 vCard 3.0 名片二维码。' },
    ...
    ogPreview: { title: 'Open Graph 预览', desc: '生成 Open Graph 和 Twitter Card 标签，并提供简单卡片预览。' },
    jsonLd: { title: 'JSON-LD 生成', desc: '引导生成 WebSite、Article、Organization 和 BreadcrumbList 的 JSON-LD。' },
