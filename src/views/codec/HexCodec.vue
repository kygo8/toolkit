...
<template>
  <div class="tool-page">
    <h1 class="page-title">🔢 HEX 编解码</h1>

    <div class="card">
      <div class="mode-tabs">
        <button class="mode-tab" :class="{ active: mode === 'encode' }" @click="mode = 'encode'; clearAll()">编码 (Encode)</button>
        <button class="mode-tab" :class="{ active: mode === 'decode' }" @click="mode = 'decode'; clearAll()">解码 (Decode)</button>
      </div>

      <div class="converter-container">
        <div class="converter-section">
          <div class="section-header">
            <span class="section-title">{{ mode === 'encode' ? '原始文本' : '十六进制字符串' }}</span>
          </div>
          <textarea
            v-model="input"
            class="converter-textarea"
            :placeholder="mode === 'encode' ? '输入要编码的文本...' : '输入十六进制，如 48 65 6c 6c 6f'"
        ...
        <div class="converter-actions">
          <button class="convert-btn" @click="convert">{{ mode === 'encode' ? '→ 编码' : '→ 解码' }}</button>
        ...
        <div class="converter-section">
          <div class="section-header">
            ...
            <button v-if="output" class="copy-btn-small" @click="copyResult">{{ copied ? '✓ 已复制' : '📋 复制' }}</button>
          </div>
