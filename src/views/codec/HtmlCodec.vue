...
<template>
  <div class="tool-page">
    <h1 class="page-title"><> HTML 编解码</h1>
      ...
      <div class="converter-container">
        <div class="converter-section">
          ...
          <textarea
            ...
            :placeholder="mode === 'encode' ? '输入要编码的文本...' : '输入 &amp;lt;div&amp;gt; 等HTML实体...'"
    ...
    <div class="reference">
      <h4>常用 HTML 实体</h4>
      <div class="ref-grid">
        <div v-for="(val, key) in htmlEntities" :key="key" class="ref-item" @click="input += val">
          <span class="ref-char">{{ key === ' ' ? '(空格)' : key }}</span>
          <span class="ref-entity">{{ val }}</span>
        </div>
      </div>
    </div>
  </div>
...
.reference { margin-top: 1.25rem; background: var(--card-bg); border: 1px solid var(--border-color); border-radius: 12px; padding: 1.25rem; }
.reference h4 { font-size: 0.9rem; font-weight: 600; margin-bottom: 0.75rem; color: var(--text-color); }
.ref-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(140px, 1fr)); gap: 0.5rem; }
.ref-item { display: flex; justify-content: space-between; align-items: center; padding: 0.375rem 0.625rem; background: var(--bg-color); border: 1px solid var(--border-color); border-radius: 4px; font-size: 0.8rem; cursor: pointer; transition: all 0.2s ease; }
.ref-item:hover { border-color: var(--primary-color); }
.ref-char { color: var(--text-color); font-weight: 600; }
.ref-entity { color: var(--primary-color); font-family: var(--font-mono); font-size: 0.75rem; }
</style>
