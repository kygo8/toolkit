<script setup>
import { ref } from 'vue'
import { formatXml, jsonToXml, minifyXml, xmlToJson } from '../../utils/xml.js'
...
const convert = () => {
  run((value) => (mode.value === 'json-to-xml' ? jsonToXml(value) : xmlToJson(value)))
}

const formatInput = () => {
  run((value) => (
    mode.value === 'json-to-xml'
...
const swap = () => {
  swapInputOutput(() => {
    mode.value = mode.value === 'json-to-xml' ? 'xml-to-json' : 'json-to-xml'
  })
}
</script>
...
<template>
  <div class="tool-page">
    <h1 class="page-title">🧩 JSON ↔ XML</h1>

    <div class="card">
      <div class="mode-tabs">
        <button class="mode-tab" :class="{ active: mode === 'json-to-xml' }" @click="switchMode('json-to-xml')">
          JSON → XML
        </button>
        <button class="mode-tab" :class="{ active: mode === 'xml-to-json' }" @click="switchMode('xml-to-json')">
          XML → JSON
        </button>
      </div>
      ...
      <div class="converter-container">
        <div class="converter-section">
          <div class="section-header">
            <span class="section-title">{{ mode === 'json-to-xml' ? '输入 JSON' : '输入 XML' }}</span>
          </div>
          <textarea
            v-model="input"
