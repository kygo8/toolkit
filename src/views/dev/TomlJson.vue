<script setup>
import { ref } from 'vue'
import { formatToml, jsonToToml, tomlToJson } from '../../utils/toml-json.js'
...
const convert = () => {
  run((value) => (mode.value === 'toml-to-json' ? tomlToJson(value) : jsonToToml(value)))
}

const formatInput = () => {
  run((value) => (
    mode.value === 'toml-to-json'
      ? formatToml(value)
...
const swap = () => {
  swapInputOutput(() => {
    mode.value = mode.value === 'toml-to-json' ? 'json-to-toml' : 'toml-to-json'
  })
}
</script>
...
<template>
  <div class="tool-page">
    ...
    <div class="card">
      <div class="mode-tabs">
        <button class="mode-tab" :class="{ active: mode === 'toml-to-json' }" @click="switchMode('toml-to-json')">
          TOML → JSON
        </button>
        <button class="mode-tab" :class="{ active: mode === 'json-to-toml' }" @click="switchMode('json-to-toml')">
          JSON → TOML
        </button>
      </div>
      ...
      <div class="converter-container">
        <div class="converter-section">
          <div class="section-header">
            <span class="section-title">{{ mode === 'toml-to-json' ? '输入 TOML' : '输入 JSON' }}</span>
          </div>
          <textarea
            v-model="input"
