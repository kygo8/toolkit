...
const convert = () => {
  run((value) => (mode.value === 'json-to-csv' ? jsonToCsv(value) : csvToJson(value)))
}
...
<template>
  <div class="tool-page">
    <h1 class="page-title">📑 JSON ↔ CSV</h1>

    <div class="card">
      <div class="mode-tabs">
        <button class="mode-tab" :class="{ active: mode === 'json-to-csv' }" @click="switchMode('json-to-csv')">
          JSON → CSV
        </button>
        <button class="mode-tab" :class="{ active: mode === 'csv-to-json' }" @click="switchMode('csv-to-json')">
          CSV → JSON
        </button>
      </div>
      ...
      <div class="converter-container">
        <div class="converter-section">
          <div class="section-header">
            <span class="section-title">{{ mode === 'json-to-csv' ? '输入 JSON' : '输入 CSV' }}</span>
            <span class="section-hint">{{ mode === 'json-to-csv' ? '对象数组' : '首行为表头' }}</span>
          </div>
          <textarea
            v-model="input"
...
.section-hint { color: var(--text-muted); font-size: 0.75rem; }
